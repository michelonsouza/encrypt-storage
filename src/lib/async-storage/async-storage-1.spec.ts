...
interface makeSutParams extends AsyncStorageOptions {
  secretKey?: string;
  noOptions?: boolean;
}
...
describe('AsyncStorage', () => {
  ...
  it('should enshure localStorage been called', async () => {
    ...
    await safeStorage.setItem(key, faker.word.words());
  ...
  it('should localStorage.getItem returns correct decrypted value', async () => {
    const safeStorage = makeSut();
    const key = faker.word.words();
    const value = { value: faker.word.words() };
  ...
  it('should calls localStorage.getItem with getItemFromPattern returns null', async () => {
    const safeStorage = makeSut();
    const pattern = faker.string.alphanumeric(8);
  ...
  it('should calls localStorage.clear', async () => {
    const safeStorage = makeSut();
    const key1 = faker.word.words();
    const key2 = faker.word.words();
    const anyValue = faker.word.words();
