...
import { AsyncEncryptStorage } from '@/classes';
...
describe('AsyncEncryptStorage 📦', () => {
  ...
  it('should calls localStorage.clear', async () => {
    ...
    const length1 = await sut.length;

    expect(localStorage.length).toBe(2);
    expect(length1).toBe(2);

    await sut.clear();
    const length2 = await sut.length;
  ...
  it('should get correct key insted of index', async () => {
    ...
    const result1 = await sut.key(0);
    const result2 = await sut.key(1);
    const result3 = await sut.key(2);
